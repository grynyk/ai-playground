...
class Server {
  ...
  private async bloggerTask(): Promise<void> {
    ...
    const user: ChatCompletionUserMessageParam = { role: CHAT_ROLE.USER, content: `${blog!.join('.')}` };
    const system: ChatCompletionSystemMessageParam = { role: CHAT_ROLE.SYSTEM, content: 
